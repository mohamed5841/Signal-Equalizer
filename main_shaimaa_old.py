...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def setup_widget_layout(self, spec_widget, target_widget):
        # تأكد من أن target_widget هو QWidget صالح
        if isinstance(target_widget, QWidget):
            layout = QVBoxLayout(target_widget)
            layout.addWidget(spec_widget)
            target_widget.setLayout(layout)  # تأكد من تعيين التخطيط بشكل صحيح
        else:
    ...
    def update_plot(self):
        ...
        if tracking_index < len(self.mode.signal.amplitude):
            ...
            self.spectrogram_widget1.plot_spectrogram(self.modified_audio[:tracking_index] , self.mode.signal.sample_rate , n_fft=self.mode.frames)
